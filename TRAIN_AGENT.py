...
def train(game):
    ...
    for episode in range(1,hyperparameters.EPISODES+500000000000):
        ...
        print("kills",kills,"avg kills", total_kills/games_played,"deaths", deaths,"Score:", cumureward, " Episode:", episode, " frames_seen:", frames_seen , " Epsilon:", agent.EPSILON)
        print(loss)
        if loss is not None:
            wandb.log({"avg kills": total_kills/games_played,"kills": kills,"Reward per episode":cumureward, "Avg reward":(np.sum(np.array(rewards))/episode), "Loss":loss})
